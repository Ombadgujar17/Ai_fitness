...
class ShoulderTracker(BaseTracker):
    def __init__(self):
        super().__init__()
        self.state = "down"
        self.DOWN_THRESHOLD = 100
        self.UP_THRESHOLD = 160
        self.DEPTH_THRESHOLD = 110
        self.SMOOTHING = 5
    ...
    def process_landmarks(self, lm):
        ...
        if self.state == "down" and smooth_angle > self.UP_THRESHOLD:
            self.state = "up"
            self.feedback_msg = "Pushing up"

        elif self.state == "up" and smooth_angle < self.DOWN_THRESHOLD:
            self.state = "down"

            full_extension = self.max_angle > self.UP_THRESHOLD
            good_depth = self.min_angle < self.DEPTH_THRESHOLD
            ...
            if full_extension and good_depth and good_balance:
                ...
                if not full_extension:
                    self.feedback_msg = "Extend arms fully"
                elif not good_depth:
                    self.feedback_msg = "Lower elbows more"
